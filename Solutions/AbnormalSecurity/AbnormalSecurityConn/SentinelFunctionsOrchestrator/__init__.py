...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    current_datetime = (datetime.datetime.strptime(stored_cases_datetime, "%Y-%m-%dT%H:%M:%SZ") + datetime.timedelta(minutes=5)).strftime("%Y-%m-%dT%H:%M:%SZ")
    
    logging.info(f"The stored_threats_datetime is: {stored_threats_datetime}, the stored_cases_datetime is: {stored_cases_datetime}, and the current_datetime is: {current_datetime}")
    
    asyncio.run(transfer_abnormal_data_to_sentinel("2024-01-05T16:25:00Z", "2024-01-05T16:25:00Z", "2024-01-05T16:30:00Z", context))
    logging.info("Orchestrator execution finished") 
    
def should_reset_date_params():
    logging.info(f"calling should_reset_date_params - the value is {RESET_ORCHESTRATION}")
...
async def consume(sentinel_connector, queue):
    while True:
        ...
        logging.info(f'printing the message from the consume function ---> {message}')
        try:
